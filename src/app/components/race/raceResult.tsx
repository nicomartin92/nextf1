...
const RaceResult = ({ result }: { result: Circuit }) => {
  return (
    <div
      ...
      className={`${styles.item} ${result.status === 'retired' ? styles.retired : ''}`}
    >
      ...
      <Image
        src={
          cars.find(c => c.name.toLowerCase() === result.Constructor.constructorId.toLowerCase())
            ?.image || ''
        }
      ...
      />
      <Link
        href={`/constructor/${result.Constructor.constructorId}?driver=${result.Driver.driverId}`}
      >
        Voir détail
      </Link>
    </div>
