...
export default function ConstructorPage({ params }: PageProps) {
  ...
  const [constructorResults, setConstructorResults] = useState<Constructor>({
    id: '',
    name: '',
    nationality: '',
  })
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : `Une erreur inconnue  /api/constructor/${constructorId}`
      )
    } finally {
      setLoading(false)
    }
    ...
    <div>
      ...
      <div className="grid gap-4">
        <Card result={constructorResults}>
          <Image
            src={
              cars.find(c => c.name.toLowerCase() === constructorResults.id.toLowerCase())?.image ||
              ''
            }
